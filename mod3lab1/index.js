function loadUniversityData(url) {
...
function simulateError() {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "invalid-url.json");
    xhr.onload = function () {
        if (xhr.status === 200) {
        }
        else {
            console.error("Ошибка загрузки данных (симуляция):", xhr.status, xhr.statusText);
            alert("Не удалось загрузить данные: " + xhr.statusText + " (симуляция ошибки)");
        }
    };
    xhr.onerror = function () {
        console.error("Сетевая ошибка (симуляция)");
        alert("Сетевая ошибка (симуляция). Попробуйте еще раз.");
    };
    xhr.send();
}
simulateError();
